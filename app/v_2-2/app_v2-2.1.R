...
ui <- page_navbar(
  ...
  nav_panel(
    title = "Kriged Variables Explorer",
    card(
      title = "Kriged H3 hexes",
      full_screen = TRUE,
      mapboxglOutput("map_kriged", height = "400px"),
    ),

    add_busy_spinner(position = "full-page", color = idea_colors_2024$lime)
  ),
...
server <- function(input, output) {
  ...
  library(data.table)  # fast ranks if needed
  ...
  #
  #   # pop_formatter <- switch(type = pop_format,
  #   #                         currency = \(x) paste0("$",round(x,0)),
  #   #                         percent = \(x) paste0(round(100*pred_estimate,1), "%"),
  #   #                         default = \(x) x
  #   #                         )
  ...
  #                                           pop_format = "default")
  blue_hex <- hcl(h = 210, c = 70, l = 55)
  orange_hex <- hcl(h = 30,  c = 70, l = 58)
  ...
  output$map_kriged <- renderMapboxgl({
    kriged_map <-       mapboxgl(
      ...
      add_fill_layer(
        ...
        source = "ssi2_layer",
        source_layer = "chng_child_dnsty_r7",
        fill_color = idea_colors$coolgray,
      ...
      add_fill_layer(
        id = "Students in poverty",
        source = "ssi2_layer",
        source_layer = "stus_in_poverty_r7",
        fill_color = "#626363",
      ...
      add_fill_layer(
        id = "3rd_Reading_res7",
        source = "ssi2_layer",
        source_layer = "assess_r7_3r",
        # fill_color = interpolate(
        #   column = "pred_estimate",
        #   values = c(.25,.75),
        #   stops  = c(idea_colors$yellow,
      ...
      add_fill_layer(
        id = "7th_math_res7",
        source = "ssi2_layer",
        source_layer = "assess_r7_7m",
        fill_color = orange_hex,
