...
get_sf_hhs_under_50000 <- function(state = NULL,
  ...
  sf_hhs_under_50000 <- get_acs(
    ...
    st_mutate_acs(
      table_name = "HOUSEHOLD INCOME IN THE PAST 12 MONTHS (IN 2023 INFLATION-ADJUSTED DOLLARS)",
...
get_tracts <- function(state = NULL,
                       county = NULL,
                       year = NULL) {
...
get_block_groups <- function(state = NULL,
                             county = NULL,
                             year = NULL) {
  
  sf_block_groups <- tigris::block_groups(
    state = state,
    county = county,
    year = year
  )
  
  return(sf_block_groups)
...
### Pull most recent ACS data (2023) via Census API
...
### Pull most recent shapefiles from Tigris (2022) via API

```{r pull_shapefiles_api}
# sf_counties <- bind_rows(
#   tigris::counties("TX", year = TIGRIS_YEAR),
#   tigris::counties("LA", year = TIGRIS_YEAR),
#   tigris::counties("FL", year = TIGRIS_YEAR),
#   tigris::counties("OH", year = TIGRIS_YEAR)
# )
# 
# sf_tracts <- df_counties_all %>%
#   select(-region) %>%
#   mutate(year = TIGRIS_YEAR) %>%
#   pmap(get_tracts) %>%
#   list_rbind() %>%
#   st_as_sf() %>%
#   st_transform(crs = "WGS84")
# 
# sf_block_groups <- df_counties_all %>%
#   select(-region) %>%
#   mutate(year = TIGRIS_YEAR) %>%
#   pmap(get_block_groups) %>%
#   list_rbind() %>%
#   st_as_sf() %>%
#   st_transform(crs = "WGS84")
```
...
### Pull shapefiles from previously pinned data

```{r pull_shapefiles_pins}
board <- board_connect(auth = "envvar")

sf_counties <- pin_read(board, "steven.macapagal/county_shapefiles") %>%
  select(GEOID_CO = GEOID,
         STATE_NAME,
         STUSPS,
         NAMELSADCO = NAMELSAD,
         NAMELSADCOSHORT = NAME,
         geometry) %>%
  left_join(df_counties_all %>%
              select(-state),
            by = c("NAMELSADCO" = "county"))

sf_tracts <- pin_read(board, "steven.macapagal/tract_shapefiles") %>%
  select(GEOID_TR = GEOID,
         STATE_NAME,
         STUSPS,
         NAMELSADCO,
         NAMELSADTR = NAMELSAD,
         NAMELSADTRSHORT = NAME,
         geometry) %>%
  left_join(df_counties_all %>%
              select(-state),
            by = c("NAMELSADCO" = "county"))

sf_block_groups <- pin_read(board, "steven.macapagal/block_group_shapefiles") %>%
  select(GEOID_BG = GEOID,
         NAMELSADBG = NAMELSAD,
         NAMELSADBGSHORT = NAME,
         geometry) %>%
  mutate(GEOID_CO = str_sub(GEOID_BG, 1, 5),
         GEOID_TR = str_sub(GEOID_BG, 1, 11)) %>%
  left_join(sf_counties %>%
              as.data.frame() %>%
              select(GEOID_CO,
                     STATE_NAME,
                     STUSPS,
                     ...
                     NAMELSADCOSHORT,
                     region),
            by = c("GEOID_CO" = "GEOID_CO")) %>%
  left_join(sf_tracts %>%
              as.data.frame() %>%
              select(GEOID_TR,
                     NAMELSADTR,
                     NAMELSADTRSHORT),
            by = c("GEOID_TR" = "GEOID_TR"))

sf_school_districts <- pin_read(board, "steven.macapagal/school_district_shapefiles")
```
...
```{r append_data}
df_acs_vars <- read_csv(here::here("etl/acs_table_lookup.csv"))

sf_acs_all <- # list_sf_students_poverty %>%
  list(
  ...
  left_join(df_counties_all %>%
              select(-state),
            by = c("county" = "county")) %>%
  left_join(df_acs_vars %>%
              select(table_number,
                     variable,
                     table_variable_number),
            by = c("table_number" = "table_number",
                   "variable" = "variable")) %>%
  mutate(year = CENSUS_YEAR) %>%
  relocate(region,
           .after = state) %>%
  relocate(table_variable_number,
           .after = GEOID) %>%
  relocate(year,
           .after = table_number) %>%
  st_as_sf() %>%
...
sfarrow::st_write_feather(sf_acs_all, here::here("app/sf_students_poverty_2023.feather"))
```
