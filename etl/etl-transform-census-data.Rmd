...
- Last deployed (development): 2023-11-08
...
sf_hh_children_under_18 <- sf_acs %>%
  ...
  # get appropriate table rows
  filter(
    table_short_name == "Households with children under 17",
    county %in% mvp_counties
  ) %>% # pull specific ACS table
  filter(!st_is_empty(.)) %>% # filter out NA geometry to prep for dot density
...
board_posit %>%
  pin_write(
    ...
    description = "Dashboard-ready dot-density ACS data",
    type = "qs"
  )
```
