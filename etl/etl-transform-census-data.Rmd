...
mvp_counties <- df_counties_all %>%
  filter(region %in% c("Austin", "Permian Basin", "Tarrant County", "Cincinnati", "Jacksonville", "Tampa")) %>%
  pull(county)
```
...
sf_household_income <- sf_acs %>%
  ...
  group_by(table_name, table_short_name, table_number, state, geography, GEOID, variable) %>%
  summarize(
    estimate = sum(estimate),
    moe = moe_sum(moe, estimate)
  ) %>%
...
sf_hh_children_under_18 <- sf_acs %>%
  ...
  # get appropriate table rows
  filter(table_short_name == "Households with children under 17",
         county %in% mvp_counties) %>% # pull specific ACS table
  filter(!st_is_empty(.)) %>% # filter out NA geometry to prep for dot density
