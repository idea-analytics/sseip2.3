...
st_as_sf_schools <- function(.data, crs = "WGS84") {
  ...
  sf_schools <- sf::st_as_sf(.data, coords = c("long", "lat")) %>%
    sf::st_set_crs("WGS84")
...
sf_students_idea_mvp <- df_counties_all %>%
  dplyr::filter(region %in% mvp_regions) %>%
...
sf_schools_idea_mvp <- df_geolocated_schools %>%
  dplyr::filter(region %in% mvp_regions) %>%
  st_as_sf_schools()
```


## Get isochrones for schools

```{r get_isochrones}
sf_isochrones_idea_mvp <- sf_schools_idea_mvp %>%
  {
    list(
