...
```{r pull_counties_states}
sf_counties_2022 <- bind_rows(
  tigris::counties("TX"),
  tigris::counties("LA"),
  tigris::counties("FL"),
  tigris::counties("OH")
...
sf_states_2022 <- states() %>%
  filter(STUSPS %in% c("TX", "LA", "FL", "OH")) %>%
  st_transform(crs = "WGS84")
```
...
sf_isochrones_idea <- sf_schools_idea %>%
  ...
  sf::st_as_sf() %>%
  sf::st_set_crs("WGS84") %>%
  st_intersection(sf_states_2022)
```
