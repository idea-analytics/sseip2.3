...
df_geolocated_schools <- read_csv(here::here("etl","GIS sites data summary.csv"))%>%
  janitor::clean_names()%>%
...
# get_geocoded_schools2 <- function(region = NULL) {
#   
#   geocoded_schools <- df_geolocated_schools %>%
#    # mutate() %>%
#     select(SchoolShortName = school_short_name, 
#            SchoolStreet = addresses, 
#            SchoolCity = city, 
#            SchoolState = state, 
#            SchoolZipCode = zip,
#            lat = latitude,
...
sf_geocoded_schools <- df_geolocated_schools %>%
  st_as_sf_schools()
```
